...
from third_party.deeplab.core import dense_prediction_cell
...
def predict_labels_multi_scale(images,
  ...
  for i, image_scale in enumerate(eval_scales):
    with tf.compat.v1.variable_scope(
        tf.compat.v1.get_variable_scope(), reuse=True if i else None):
...
def multi_scale_logits(images,
  ...
  Args:
    ...
    output_end_points: Whether to output encoder endpoints.
  ...
  if output_end_points and len(image_pyramid) != 1:
    target_end_points = None
  ...
  for image_scale in image_pyramid:
    ...
    if not output_end_points:
      ...
      outputs_to_logits, end_points = _get_logits(
          scaled_images,
          updated_options,
          weight_decay=weight_decay,
          reuse=tf.AUTO_REUSE,
          ...
          fine_tune_batch_norm=fine_tune_batch_norm,
          nas_training_hyper_parameters=nas_training_hyper_parameters,
          output_end_points=output_end_points)
      if image_scale == 1.0:
        target_end_points = end_points
    ...
    if len(image_pyramid) == 1:
      ...
      if output_end_points:
        return outputs_to_scales_to_logits, end_points
      else:
  ...
  if output_end_points:
    return outputs_to_scales_to_logits, target_end_points
  else:
    return outputs_to_scales_to_logits
...
def _get_logits(images,
  ...
  if output_end_points:
    return outputs_to_logits, end_points
  else:
...
def get_branch_logits(features,
  ...
  Args:
    ...
    use_bounded_activation: Whether to use the bounded activation. If True, a
      bandpass function of (-6, 6) will be applied as activation.
  ...
  with slim.arg_scope(
      ...
      weights_initializer=tf.compat.v1.initializers.truncated_normal(
          stddev=0.01),
      reuse=reuse):
    with tf.compat.v1.variable_scope(LOGITS_SCOPE_NAME, LOGITS_SCOPE_NAME,
                                     [features]):
      branch_logits = []
      for i, rate in enumerate(atrous_rates):
        ...
        bandpass_fn = lambda x: tf.clip_by_value(x, -6, 6)
        branch_logits.append(
            slim.conv2d(
                ...
                rate=rate,
                activation_fn=bandpass_fn if use_bounded_activation else None,
                normalizer_fn=None,
                scope=scope))
